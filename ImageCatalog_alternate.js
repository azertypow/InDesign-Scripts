...
/*  
@@@BUILDINFO@@@ "ImageCatalog_alternate.jsx" 3.0.0 15 April 2019
*/

main();

function main() {
	var numberOfFolders = numberOfFoldersDialog();

	if (numberOfFolders != null && numberOfFolders > 0) {
		...
		for (var i = 1; i <= numberOfFolders; i++) {
			...
			userFolders.push(Folder.selectDialog("Select the folder containing the images : n°" + i, ""));
		}

		for (var i = 0; i <= userFolders.length -1; i++) {
			// Filter the files
			var filteredFiles;
			if (File.fs === "Macintosh") {
				filteredFiles = myMacOSFileFilter(userFolders[i]);
			}
			else {
				filteredFiles = myWinOSFileFilter(userFolders[i]);
			}

			folderOptionDialog(filteredFiles, userFolders[i], i);
		}
	}
...
function numberOfFoldersDialog() {
	var myLabelWidth = 112;
	var myDialog = app.dialogs.add({ name: "Image Catalog : Alternate" });

	with (myDialog.dialogColumns.add()) {

		with (dialogRows.add()) {
			staticTexts.add({ staticLabel: "Choose a number of different image folders.", minWidth: myLabelWidth });
		}
		with (dialogRows.add()) {
			staticTexts.add({ staticLabel: "Their images are going to be displayed alternatively on each page.", minWidth: myLabelWidth });
		}
		with (dialogRows.add()) {
			staticTexts.add({ staticLabel: "", minWidth: myLabelWidth });
		}
		with (dialogRows.add()) {
			staticTexts.add({ staticLabel: "Number of Folders:", minWidth: myLabelWidth });
			var numberOfFoldersField = integerEditboxes.add({ editValue: 2 });
		}
		...
		if (myResult == true) {
			numberOfFolders = numberOfFoldersField.editValue;
...
function folderOptionDialog(userFiles, userFolder, folderNumber) {
	var myLabelWidth = 112;
	var myDialog = app.dialogs.add({ name: "Options for Folder n°" +  (folderNumber + 1)});
		...
		with (borderPanels.add()) {
			with (dialogColumns.add()) {
				with (dialogRows.add()) {
					...
					staticTexts.add({ staticLabel: userFolder.path + "/" + userFolder.name });
				}
				with (dialogRows.add()) {
					staticTexts.add({ staticLabel: "Number of Images:", minWidth: myLabelWidth });
					staticTexts.add({ staticLabel: userFiles.length + "" });
				}
			}
		}
		with (dialogRows.add()) {
